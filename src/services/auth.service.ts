...
import { IAuthBuyerMessageDetails, IAuthDocument, firstLetterUppercase, lowerCase, winstonLogger } from '@dovgutman/jobber-shared';
...
export async function getAuthUserById(authId: number): Promise<IAuthDocument | undefined> {
  try {
    const user: Model = (await AuthModel.findByPk(authId, {
      attributes: {
        exclude: ['password']
      }
    })) as Model;
